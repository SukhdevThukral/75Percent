...
import { saveClasses, setOnboarded, assignColors, saveName } from '@/utils/classes'
...
export default function Onboarding() {
    ...
    const [step, setStep] = useState<'name' | 'timetable'>('name')
                ...
                {step === 'name' ? (
                    <>
                        <Text style={tw`text-white text-4xl font-extrabold tracking-tight text-center`}>
                            What's your{'\n'}<Text style={tw`text-[#7C3AED]`}>name?</Text>
                        </Text>
                        <TextInput
                            value={name}
                            ...
                            style={tw`w-full bg-[#1C1C1E] text-white px-4 py-4 rounded-2xl text-base`}
                        />
                        <TouchableOpacity
                            onPress={() => { if (name.trim()) setStep('timetable') }}
                            style={tw`w-full bg-[#7C3AED] py-4 rounded-2xl items-center`}
                        >
